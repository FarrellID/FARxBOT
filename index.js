...
app.get('/', (req, res) => res.send('[🤖] BOTz Successfully Online!!'))
...
client.on("guildMemberAdd", async (member) => {
  ...
  if (!LoggingChannel)return console.log(`Setup Belum Selesai di ${member.guild.id} alias ${member.guild.name} Guild ( Channel tidak Ditemukan )`);

  //getting notify role
  let notifyRole = await db.get(`notifyRole_${member.guild.id}`);
  if (!notifyRole)return console.log(`Setup Belum Selesai di ${member.guild.id} alias ${member.guild.name} Guild ( Role tidak Ditemukan )`);
  ...
  if (created < AltAge) {
    ...
    new Discord.MessageEmbed().setTitle("Alt Found!").setColor("RANDOM").setFooter("BOTz Made by **Parreelll #7172**")
      .setDescription(`
...
if (!AutoKick)return console.log(`Setup Belum Selesai di ${member.guild.id} alias ${member.guild.name} Guild ( AutoKick tidak Diaktifkan )`);
...
if (!AutoKickAge) return db.set(`AutokickAge_${member.guild.id}`, 8)
 ...
 let checking = await db.get(`WhiteListed_${member.guild.id}`)
  ...
  if (checking === member.user.id) {
   ...
   .setTitle(`AutoKick System Otomatis Stuck On`)
   .setDescription(`
...
member.guild.channels.cache.get(LoggingChannel).send(embed)
    ...
    let embed = new Discord.MessageEmbed()
    .setTitle(`AutoKick System Otomatis Menendang Seseorang`)
    .setDescription(`
**__NAME__** - ${member.user} (${member.user.username})
**__ID__** - ${member.user.id}
...
`)
    ...
    console.log(`AutoKick tidak Dinonaktifkan di ${memeber.guild.name}`)
...
return message.channel.send(`**:x: Tolong Jangan TAG Siapapun**`);
 }
  message.channel.startTyping();
if (!message.content) return message.channel.send("Tolong katakan sesuatu.");
...
client.on("ready", () => {
    ...
    console.log("[❗] BOTz sedang Berjalan pada Server...")
});
...
const fs = require('fs')
 ...
 client.on('guildCreate', guild =>{
    ...
    const channelId = '958894249078841375'; //put your channel ID here

    const channel = client.channels.cache.get(channelId); 
     
    if(!channel) return; //If the channel is invalid it returns
    const embed = new discord.MessageEmbed()
        .setTitle('Saya Bergabung dengan Guild!')
...
client.on('guildDelete', guild =>{
    const channelId = '958894249078841375';//add your channel ID
    const channel = client.channels.cache.get(channelId); //This Gets That Channel
    
    if(!channel) return;  //If the channel is invalid it returns
    const embed = new discord.MessageEmbed()
        .setTitle('Saya Meninggalkan Guild!')
        .setDescription(`**Guild Name:** ${guild.name} (${guild.id})\n**Members:** ${guild.memberCount}`)
        .setTimestamp()
        .setColor('RED')
        .setFooter(`I'm in ${client.guilds.cache.size} Guilds Now!`);
...
const step1 = new MessageEmbed()
  ...
  .addField(
    "<:857122481088495629:873454677231034368> a", "Our first step is to get the webpage link. You can find the code in the bottom or side of you repl.it(see screenshot below)! If you do not have this link, copy paste this code at the top of your `index.js` and run it again.\n ```https://pastebin.com/HJGhAUZf```"
  )
